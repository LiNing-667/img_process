...
namespace CameraManager
{
    ...
    bool forceCameraFormat(int dev_id, bool silent = false)
    {
        ...
        {
            if (!silent)
                std::cerr << "[底层强控] 警告：格式强制指令失败！" << std::endl;
        }
        else
        {
            if (!silent)
                std::cout << "[底层强控] 请求 1280x720 MJPG。硬件分配: " << fmt.fmt.pix.width << "x" << fmt.fmt.pix.height << std::endl;
        }
    ...
    bool getLatestFrame(SharedFrame &shared, Mat &frame, int timeout_ms)
    {
        ...
        if (!shared.cv.wait_for(lock, chrono::milliseconds(timeout_ms), [&shared]
                                { return shared.ready; }))
            return false;
        if (shared.frame.empty())
            return false;
...
public:
    ...
    int acceptClient()
    {
        ...
        if (client_socket < 0)
            return -1; // 非阻塞模式下，没人连会直接 return -1，不卡主线程
...
std::vector<Point2f> runNextYoloInferenceRaw(const Mat &roi_frame)
{
    ...
    Mat resized_frame;
    cv::resize(roi_frame, resized_frame, Size(new_w, new_h));
    Mat padded_frame;
        ...
        else
        {
            ...
            if (found_target_obj)
            {
                ...
                if (dx_pixel > 0.0f)
                {
                    ...
                    cout << "    [伺服决策] 目标框偏右，机械臂 Y 轴减小 2.0mm" << endl;
                }
                else
                {
                    g_cl_state.last_pose.y += STEP_CM;
                    cout << "    [伺服决策] 目标框偏左，机械臂 Y 轴增大 2.0mm" << endl;
                }
                if (dy_pixel > 0.0f)
                {
                    g_cl_state.last_pose.x -= STEP_CM;
                    cout << "    [伺服决策] 目标框偏下，机械臂 X 轴减小 2.0mm" << endl;
                }
                else
                {
                    g_cl_state.last_pose.x += STEP_CM;
                    cout << "    [伺服决策] 目标框偏上，机械臂 X 轴增大 2.0mm" << endl;
                }
            }
            else
            {
